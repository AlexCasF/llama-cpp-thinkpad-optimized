...
$profileName = "HP Z6 G4 / Qwen 3.6 35B CPU-only"
...
$runtimeFlavor = "Windows CPU"
$runtimeRoot = Join-Path $env:LOCALAPPDATA "llama-server-kit\runtime"
$runtimeDirectoryName = "$BuildTag-cpu"
$runtimeDir = Join-Path $runtimeRoot $runtimeDirectoryName
$runtimeArchive = Join-Path $runtimeRoot "llama-$BuildTag-bin-win-cpu-x64.zip"
$runtimeAssetName = "llama-$BuildTag-bin-win-cpu-x64.zip"
$releaseUri = "https://api.github.com/repos/ggml-org/llama.cpp/releases/tags/$BuildTag"
$runtimeCheckExeName = "llama-server.exe"
$runtimeCheckArgs = @("--version")
$requiredRuntimeFiles = @("llama-server.exe")
$useLevelZero = $false
